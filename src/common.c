...
int parsehost(int family, unsigned char *host, struct sockaddr *sa)
{
	...
	if (*host == '[')
		se = strchr((char *)host, ']');
	if ((sp = strchr(se ? se : (char *)host, ':')) && !strchr(sp + 1, ':'))
		*sp = 0;
	if (se)
	{
		*se = 0;
...
int parseconnusername(char *username, struct clientparam *param, int extpasswd, unsigned short port)
{
	...
	if ((sb = strchr(username, conf.delimchar)) == NULL)
	{
		if (!param->hostname && param->remsock == INVALID_SOCKET)
			return 2;
		if (param->hostname)
			parsehostname((char *)param->hostname, param, port);
		return parseusername(username, param, extpasswd);
	}
	while ((se = strchr(sb + 1, conf.delimchar)))
		sb = se;
	*(sb) = 0;
	if (parseusername(username, param, extpasswd))
		return 3;
	*(sb) = conf.delimchar;
	if (parsehostname(sb + 1, param, port))
		return 4;
	return 0;
}
...
int doconnect(struct clientparam *param)
{
	...
	if (*SAFAMILY(&param->sincl) == *SAFAMILY(&param->req) && !memcmp(SAADDR(&param->sincl), SAADDR(&param->req), SAADDRLEN(&param->req)) &&
		*SAPORT(&param->sincl) == *SAPORT(&param->req))
		return 519;
	...
	if (param->remsock != INVALID_SOCKET)
	{
		...
		if (so._getpeername(param->remsock, (struct sockaddr *)&param->sinsr, &size) == -1)
		{
			return (14);
		}
	}
	else
	{
		...
		if (!*SAPORT(&param->sinsr))
			*SAPORT(&param->sinsr) = *SAPORT(&param->req);
		if ((param->remsock = so._socket(SASOCK(&param->sinsr), SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET)
		{
			return (11);
		}
		...
		{
#ifndef NOIPV6
			if (*SAFAMILY(&param->sinsr) == AF_INET6)
				param->sinsl = param->srv->extsa6;
			else
#endif
		...
		*SAPORT(&param->sinsl) = 0;
		...
		if (param->srv->obindtodevice)
		{
			...
			if (!idx || (*SAFAMILY(&param->sinsl) == AF_INET && so._setsockopt(param->remsock, IPPROTO_IP, IP_BOUND_IF, &idx, sizeof(idx))))
				return 12;
#ifndef NOIPV6
			if (*SAFAMILY(&param->sinsl) == AF_INET6 && so._setsockopt(param->remsock, IPPROTO_IPV6, IPV6_BOUND_IF, &idx, sizeof(idx)))
				return 12;
#endif
		}
#endif
		...
		if (so._getsockname(param->remsock, (struct sockaddr *)&param->sinsl, &size) == -1)
		{
			return (15);
		}
	}
...
int scanaddr(const unsigned char *s, unsigned long *ip, unsigned long *mask)
{
	...
	int res;
	if ((res = sscanf((char *)s, "%u.%u.%u.%u/%u", &d1, &d2, &d3, &d4, &m)) < 4)
		return 0;
	if (mask && res == 4)
		*mask = 0xFFFFFFFF;
	else if (mask)
...
#define gethostbyname(NAME) my_gethostbyname(NAME, ghbuf, &he)
	...
	{
		if ((*tmpresolv)(AF_INET, name, (unsigned char *)&retval))
			return retval;
