	...
	for (i = 0, j = 0; format[j] && i < 4040; j++)
	{
		if (format[j] == '%' && format[j + 1])
		{
			...
			case 'U':
				if (param->username && *param->username)
				{
					for (len = 0; i < 4000 && param->username[len]; len++)
					{
						...
						if (param->srv->nonprintable && (buf[i] < 0x20 || strchr((char *)param->srv->nonprintable, buf[i])))
							buf[i] = param->srv->replace;
			...
			case 'n':
				...
				if (len > 0 && !strchr((char *)param->hostname, ':'))
					for (len = 0; param->hostname[len] && i < 4000; len++, i++)
					{
...
int dobuf(struct clientparam *param, unsigned char *buf, const unsigned char *s, const unsigned char *doublec)
{
	...
	if (!param)
		return 0;
	if (param->trafcountfunc)
		(*param->trafcountfunc)(param);
	format = param->srv->logformat ? (char *)param->srv->logformat : DEFLOGFORMAT;
	tm = (*format == 'G' || *format == 'g') ? gmtime(&t) : localtime(&t);
...
void logstdout(struct clientparam *param, const unsigned char *s)
{
	...
	log = param->srv->stdlog ? param->srv->stdlog : conf.stdlog ? conf.stdlog
																: stdout;
	if (!param->nolog)
		if (fprintf(log, "%s\n", tmpbuf) < 0)
		{
