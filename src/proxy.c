...
static void logurl(struct clientparam *param, char *buf, char *req, int ftp)
{
	...
	if (param->res != 555 && param->res != 508)
		dolog(param, (unsigned char *)(req ? buf : NULL));
}
...
void *proxychild(struct clientparam *param)
{
	...
	for (;;)
	{
		...
		if (buf[i - 3] == '1')
			keepalive = 2;
		param->transparent = 0;
		if ((isconnect = !strncasecmp((char *)buf, "CONNECT", 7)))
			keepalive = 2;

		if ((sb = (unsigned char *)(unsigned char *)strchr((char *)buf, ' ')) == NULL)
		{
			RETURN(512);
		}
		ss = ++sb;
		...
		{
			if ((se = (unsigned char *)(unsigned char *)strchr((char *)sb, ' ')) == NULL || sb == se)
			{
				RETURN(514);
			}
			*se = 0;
		...
		if (!param->transparent || isconnect)
		{
			if (!isconnect)
			{
				if ((se = (unsigned char *)(unsigned char *)strchr((char *)sb, '/')) == NULL || sb == se || !(sg = (unsigned char *)strchr((char *)sb, ' ')))
				{
					RETURN(515);
				}
				if (se > sg)
		...
		if (!strncasecmp((char *)buf, "CONNECT", 7))
			param->operation = HTTP_CONNECT;
		else if (!strncasecmp((char *)buf, "GET", 3))
			param->operation = (ftp) ? FTP_GET : HTTP_GET;
		else if (!strncasecmp((char *)buf, "PUT", 3))
			param->operation = (ftp) ? FTP_PUT : HTTP_PUT;
		else if (!strncasecmp((char *)buf, "POST", 4) || !strncasecmp((char *)buf, "BITS_POST", 9))
			param->operation = HTTP_POST;
		else if (!strncasecmp((char *)buf, "HEAD", 4))
			param->operation = HTTP_HEAD;
		else
		...
		do
		{
							   ...
							   (i > 16 && (!strncasecmp((char *)(buf + inbuf), "connection:", 11)))))
			{
				...
				{
					if (!strncasecmp((char *)sb, "keep-alive", 10))
						keepalive = 1;
					else
						keepalive = 0;
					continue;
			...
			if ((bufsize - inbuf) < LINESIZE)
			{
				...
				if (!(newbuf = myrealloc(buf, bufsize + BUFSIZE)))
				{
					RETURN(21);
				}
				buf = newbuf;
		...
		if (ftp && param->redirtype != R_HTTP)
		{
			...
			*sp = 0;
			...
			if (!mode)
			{
				...
				s = param->remsock;
				param->remsock = ftps;
				if ((param->operation == FTP_PUT) && (contentlength64 > 0))
		...
		if (!anonymous)
		{
			...
			if (*SAFAMILY(&param->sincr) == AF_INET6)
				len += sprintf((char *)buf + len, "]:%d\";by=", (int)ntohs(*SAPORT(&param->sincr)));
			else
				len += sprintf((char *)buf + len, ":%d;by=", (int)ntohs(*SAPORT(&param->sincr)));
