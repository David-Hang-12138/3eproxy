...
int sockgetcharcli(struct clientparam *param, int timeosec, int timeousec)
{
	...
	if (!param->clibuf)
	{
		if (!(param->clibuf = myalloc(SRVBUFSIZE)))
			return 0;
	...
	param->clioffset = param->cliinbuf = 0;
	if ((len = sockrecvfrom(param->clisock, (struct sockaddr *)&param->sincr, param->clibuf, param->clibufsize, timeosec * 1000 + timeousec)) <= 0)
		return EOF;
	param->cliinbuf = len;
	param->clioffset = 1;
...
int sockgetcharsrv(struct clientparam *param, int timeosec, int timeousec)
{
	...
	{
		bufsize = SRVBUFSIZE;
		if (param->ndatfilterssrv > 0 && bufsize < 32768)
			bufsize = 32768;
		if (!(param->srvbuf = myalloc(bufsize)))
			return 0;
	...
	param->srvoffset = param->srvinbuf = 0;
	if ((len = sockrecvfrom(param->remsock, (struct sockaddr *)&param->sinsr, param->srvbuf, param->srvbufsize, timeosec * 1000 + timeousec)) <= 0)
		return EOF;
	param->srvinbuf = len;
	param->srvoffset = 1;
	param->nreads++;
