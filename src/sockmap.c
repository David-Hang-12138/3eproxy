...
int sockmap(struct clientparam *param, int timeo, int usesplice)
{
	...
	if (param->operation == UDPASSOC || (!param->nolongdatfilter && (param->ndatfilterscli > 0 || param->ndatfilterssrv)))
		usesplice = 0;
	if (usesplice)
	{
		TOCLIENTPIPE = FROMCLIENTPIPE = TOSERVERPIPE = FROMSERVERPIPE = 1;
		TOCLIENTBUF = TOSERVERBUF = 0;
		if (pipe2(pipecli, O_NONBLOCK) < 0)
			RETURN(21);
		if (pipe2(pipesrv, O_NONBLOCK) < 0)
			RETURN(21);
	}
#endif
	...
	if (inserverbuf >= fromserver)
		FROMSERVER = 0;
	if (inclientbuf >= fromclient)
		FROMCLIENT = 0;
#ifdef WITHSPLICE
	if (!usesplice)
#endif
	...
	if (param->clibufsize == param->cliinbuf)
		TOCLIENTBUF = 0;
	if (param->srvbufsize == param->srvinbuf)
		TOSERVERBUF = 0;
										 ...
										 || (!CLIENTTERM))))
	{
		...
		{
			if (needaction > (MAXFAILATTEMPT + 1))
			{
				RETURN(93);
			}
		...
		{
			if (!param->srv->noforce && (res = (*param->srv->authfunc)(param)) && res != 2)
			{
				RETURN(res);
			}
		...
		if (inclientbuf && TOSERVER)
		{
			...
			else
			{
				...
				if (param->clioffset == param->cliinbuf)
					param->clioffset = param->cliinbuf = 0;
				if (param->cliinbuf < param->clibufsize)
					TOCLIENTBUF = 1;
		...
		if (inserverbuf && TOCLIENT)
		{
			...
			else
			{
				...
				if (param->srvinbuf < param->srvbufsize)
					TOSERVERBUF = 1;
				needaction = 0;
				continue;
			}
			...
			if (!CLIENTTERM)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							if (fds[fdsc].events & POLLIN)
								FROMCLIENT = 1;
							if (fds[fdsc].events & POLLOUT)
								CLIENTTERM = 1;
						}
					}
				}
	...
	res = 0;
	if (!fromserver && param->waitserver64)
		res = 98;
	else if (!fromclient && param->waitclient64)
		res = 99;
	else if (HASERROR)
		res = 94 + HASERROR;
	else if ((inclientbuf || inserverbuf))
		res = 94;
#ifdef WITHSPLICE
	else if (inclientpipe || inserverpipe)
		res = 94;
#endif
